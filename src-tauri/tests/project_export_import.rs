...
fn test_project_export_import_roundtrip() {
    ...
    let sc =
        featherworks_author::storage::database::create_scene(&conn, &ch.id, "Szene X").unwrap();
    featherworks_author::storage::database::update_scene_content(
        &conn,
        &sc.id,
        "Ein kurzer Text mit Worten",
        5,
    )
    .unwrap();
    ...
    let opened =
        featherworks_author::storage::database::open_database(tmp.path().to_str().unwrap())
            .unwrap();
    // Basic assertions
    featherworks_author::storage::database::ensure_project_exists(&opened, "Titel", "Autor")
        .unwrap();
