...
fn encrypted_project_export_import_flow() {
    ...
    let sc = featherworks_author::storage::database::create_scene(&conn, &ch.id, "Szene 2").unwrap();
    featherworks_author::storage::database::update_scene_content(&conn, &sc.id, "Ein kurzer Text", 3).unwrap();
    ...
    let tmp_conn = featherworks_author::storage::database::open_database(tmp.path().to_str().unwrap()).unwrap();
    // heal project row if missing
    featherworks_author::storage::database::ensure_project_exists(&tmp_conn, "Titel X", "Autor Y").unwrap();
    ...
    assert!(chapters.len() >= 2, "expected at least 2 chapters after import, got {}", chapters.len());
}
