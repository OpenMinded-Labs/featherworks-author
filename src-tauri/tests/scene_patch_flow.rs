...
fn scene_patch_undo_redo_flow() {
    ...
    let (json_opt, plain, wc2) = database::get_scene_content_with_json(&conn, scene_id).expect("get scene content json");
    ...
    let wc_after_second = scenes_service::apply_patch(&conn, scene_id, p2).expect("apply 2nd patch");
    assert_eq!(wc_after_second, 3);

    // Undo (should restore first version)
    let undo_wc = scenes_service::undo(&conn, scene_id).expect("undo").expect("expected Some");
    assert_eq!(undo_wc, 2);
    let (json_after_undo, plain_after_undo, wc_after_undo) = database::get_scene_content_with_json(&conn, scene_id).expect("get after undo");
    ...
    let redo_wc = scenes_service::redo(&conn, scene_id).expect("redo").expect("expected Some");
    assert_eq!(redo_wc, 3);
    let (_json_after_redo, plain_after_redo, wc_after_redo) = database::get_scene_content_with_json(&conn, scene_id).expect("get after redo");
