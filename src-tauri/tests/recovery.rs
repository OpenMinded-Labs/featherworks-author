...
use featherworks_author::services::{recovery_service, scenes_service};
...
fn recovery_reconstructs_scene() {
    ...
    for i in 0..30u32 {
        let txt = format!("{i} ");
        let p = patch::Patch(patch::PatchKind::InsertText {
            offset: 0,
            text: txt,
        });
    ...
    let rec_scene = report
        .scenes
        .iter()
        .find(|s| s.scene_id == *scene_id)
        .expect("scene recovered");
