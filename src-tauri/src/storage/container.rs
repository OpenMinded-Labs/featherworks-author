...
impl ContainerHeader {
    ...
    fn read(mut f:&fs::File) -> anyhow::Result<Self> {
        let mut magic=[0u8;7]; f.read_exact(&mut magic)?; if &magic!=MAGIC { anyhow::bail!("invalid magic") }
        let mut ver=[0u8;1]; f.read_exact(&mut ver)?;
        if ver[0]!=VERSION { anyhow::bail!("unsupported version") }
        let mut meta_len_bytes=[0u8;4]; f.read_exact(&mut meta_len_bytes)?;
        let meta_len = u32::from_be_bytes(meta_len_bytes) as usize;
        let mut meta_buf=vec![0u8;meta_len]; f.read_exact(&mut meta_buf)?;
        ...
        let (salt, m_cost, t_cost, p_cost, nonce) = if meta_str.trim_start().starts_with('{') {
            ...
            let salt = v["salt"].as_str().ok_or_else(|| anyhow!("salt missing"))?.to_string();
            ...
            let nonce_vec = B64.decode(v["nonce"].as_str().ok_or_else(|| anyhow!("nonce missing"))?).context("nonce b64")?;
            let mut nonce=[0u8;12]; nonce.copy_from_slice(&nonce_vec[..12]);
            (salt, m_cost, t_cost, p_cost, nonce)
        } else {
            let parts:Vec<&str> = meta_str.split(';').collect();
            if parts.len()<4 { anyhow::bail!("corrupt legacy header meta") }
            ...
            let mut m=64*1024; let mut t=3; let mut p=1;
            for kv in argon_params.split(';') { if let Some((k,v))=kv.split_once('=') { match k { "m"=> m=v.parse().unwrap_or(m), "t"=> t=v.parse().unwrap_or(t), "p"=> p=v.parse().unwrap_or(p), _=>{} } } }
            (salt, m, t, p, nonce)
        };
        let mut len_bytes=[0u8;8]; f.read_exact(&mut len_bytes)?; let uncompressed_len=u64::from_be_bytes(len_bytes);
        Ok(Self { version: ver[0], salt, m_cost, t_cost, p_cost, nonce, uncompressed_len })
    }
}
...
fn derive_key(password:&str, salt:&[u8], params:&Params) -> anyhow::Result<[u8;32]> {
    ...
    let mut key=[0u8;32];
    argon.hash_password_into(password.as_bytes(), salt, &mut key).map_err(|e| anyhow!("argon2 kdf: {e}"))?;
    Ok(key)
}

pub fn save_encrypted(sqlite_path:&Path, out_path:&Path, password:&str) -> anyhow::Result<()> {
    let mut raw = Vec::new(); fs::File::open(sqlite_path)?.read_to_end(&mut raw)?;
    ...
    let ciphertext = cipher.encrypt(GenericArray::from_slice(&nonce), compressed.as_ref()).map_err(|e| anyhow!("encrypt: {e}"))?;
    let header = ContainerHeader { version: VERSION, salt: salt_b64, m_cost: params.m_cost(), t_cost: params.t_cost(), p_cost: params.p_cost(), nonce, uncompressed_len };
    ...
    let mut w = &f; w.write_all(&ciphertext)?;
    Ok(())
}
...
pub fn load_encrypted(path:&Path, password:&str) -> anyhow::Result<NamedTempFile> {
    ...
    let params = Params::new(header.m_cost, header.t_cost, header.p_cost, Some(32)).map_err(|e| anyhow!("argon2 params parse: {e}"))?;
    let salt_dec = B64.decode(&header.salt)?;
    let key = derive_key(password, &salt_dec, &params)?;
    let cipher = Aes256Gcm::new(GenericArray::from_slice(&key));
    let decompressed = cipher.decrypt(GenericArray::from_slice(&header.nonce), ciphertext.as_ref()).map_err(|e| anyhow!("decrypt: {e}"))?;
    let mut key_for_zero = key;
    key_for_zero.zeroize();
    let raw = lz4::decompress_size_prepended(&decompressed)?;
    if raw.len() as u64 != header.uncompressed_len { log::warn!("length mismatch: header {} vs actual {}", header.uncompressed_len, raw.len()); }
    let tmp = NamedTempFile::new()?; fs::write(tmp.path(), raw)?; Ok(tmp)
}
