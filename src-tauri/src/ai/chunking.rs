...
fn find_sentence_boundary(text: &str, target: usize) -> usize {
    ...
    for (i, c) in search_text.char_indices() {
        ...
        let is_boundary = matches!(c, '.' | '!' | '?' | '\n')
            && search_text.get(i + 1..i + 2).map(|s| s.starts_with(' ') || s.starts_with('\n')).unwrap_or(true);
...
pub fn chunk_entity(entity_id: &str, entity_name: &str, entity_type: &str, description: &str) -> Vec<TextChunk> {
