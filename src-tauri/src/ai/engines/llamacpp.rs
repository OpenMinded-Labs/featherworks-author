...
use {
    ...
    llama_cpp_2::model::{AddBos, LlamaModel, Special},
...
impl LlamaCppEngine {
    ...
    fn create_context(&self) -> Result<LlamaContext<'_>> {
        ...
        log::info!(
            "[fontaine/llamacpp] Creating context with n_ctx={}, n_batch=8192",
            ctx_size
        );
    ...
    fn generate_real(&self, prompt: &str, max_tokens: usize) -> Result<Vec<String>> {
        ...
        log::info!(
            "[fontaine/llamacpp] Prompt (test_mode={}): {}",
            test_mode,
        ...
        let tokens_list = model
            .str_to_token(&actual_prompt, AddBos::Always)
            .with_context(|| "Failed to tokenize prompt")?;

        log::info!(
            "[fontaine/llamacpp] Prompt tokenized: {} tokens",
            tokens_list.len()
        );
        ...
        log::info!(
            "[fontaine/llamacpp] Using temp={} top_p={} repeat_penalty={} seed={}",
            temperature,
            top_p,
            repeat_penalty,
            seed
            ...
            if let Ok(bytes) = model.token_to_bytes(token, Special::Tokenize) {
                ...
                if !output_string.is_empty() {
                    ...
                    if output_tokens.len() <= 30 {
                        log::info!(
                            "[fontaine/llamacpp] Token {} (id={}): '{}' bytes={:?}",
                            output_tokens.len(),
                            token.0,
                            output_string,
                            &bytes
        ...
        log::info!(
            "[fontaine/llamacpp] Full response ({} tokens): {}",
            output_tokens.len(),
...
impl ModelLoader for LlamaCppEngine {
    ...
    fn load(&mut self, info: &ModelInfo, model_path: &Path) -> Result<()> {
        ...
        log::info!(
            "[fontaine/llamacpp] Loading model '{}' from: {}",
            info.id,
            model_path.display()
        );
        ...
        log::info!(
            "[fontaine/llamacpp] Model '{}' loaded successfully",
            info.id
        );
        Ok(())
    }
    ...
    fn generate_tokens(
        ...
        #[cfg(feature = "local-llm")]
        {
            if self.model.is_some() {
                ...
                return Err(anyhow!(
                    "LLM model not loaded. Please wait for model initialization or check logs."
                ));
            }
        }
...
fn generate_simulated_response(prompt: &str, max_tokens: usize) -> Vec<String> {
    ...
    } else if !is_chat_mode && (prompt_lower.contains("agent") || prompt_lower.contains("proaktiv"))
    {
        tokens.extend(vec![
            ...
            " Die".to_string(),
            " Szene".to_string(),
            " entwickelt".to_string(),
            " sich".to_string(),
            " gut.\n\n".to_string(),
            "**Charaktere:**".to_string(),
            " Prüfe".to_string(),
            " Konsistenz".to_string(),
            " der".to_string(),
            " Stimmen.\n\n".to_string(),
            "**Vorschlag:**".to_string(),
            " Mehr".to_string(),
    ...
    } else if prompt_lower.contains("entities")
        ...
        || prompt_lower.contains("extrahiere")
    {
        ...
        if user_message.contains("wer bist du")
            || user_message.contains("wer bist")
            || user_message.contains("stell dich vor")
        {
            tokens.extend(vec![
                "Hallo! ".to_string(),
                "Ich".to_string(),
        ...
        } else if user_message.contains("hallo")
            || user_message.contains("hey")
            || user_message.contains("hi")
        {
            tokens.extend(vec![
                "Hallo! ".to_string(),
                "👋".to_string(),
