...
static VAMPIRE_VERBS: &[&str] = &[
    "war", "waren", "hatte", "hatten", "wurde", "wurden", "gewesen", "geworden", "sein", "ist",
    "sind",
];
...
pub fn analyze_text_style(text: &str) -> StyleAnalysis {
    ...
    for w in &words {
        ...
        if VAMPIRE_VERBS.contains(&lw.as_str()) {
            found_vampires.push(lw);
        }
    }
    ...
    let repetitions = counts
        .iter()
        .filter(|(w, c)| w.len() > 4 && **c > 3)
        .map(|(w, _)| w.clone())
        .collect();
    ...
    let dialog_ratio = if words.is_empty() {
        0.0
    } else {
        (dialog_tokens as f32 / words.len() as f32).min(1.0)
    };
    let complexity = sentence_complexity(text);
    let readability_score = readability(text);
    StyleAnalysis {
        vampire_verbs: found_vampires,
...
fn sentence_complexity(text: &str) -> f32 {
    let s = text
        .split(&['.', '!', '?'][..])
        .filter(|p| p.trim().len() > 0)
        .count() as f32;
    let w = text.split_whitespace().count() as f32;
    if s > 0.0 {
        w / s
    } else {
        0.0
...
fn readability(text: &str) -> f32 {
    85.0 - (sentence_complexity(text) * 0.5)
}
