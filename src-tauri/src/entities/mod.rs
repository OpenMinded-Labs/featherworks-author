...
pub fn configure_entities(named: &[(String, String)]) {
    // (id,name)
    {
...
fn rebuild() {
    if let Ok(pats) = ENTITY_PATTERNS.read() {
        if let Ok(mut set) = ENTITY_SET.write() {
            *set = RegexSet::new(pats.iter().map(|p| p.regex.as_str())).ok();
        }
    }
}
...
pub fn scan(text: &str) -> Vec<EntityHit> {
    ...
    if let Some(set) = &*set_opt {
        for idx in set.matches(text).into_iter() {
            if let Some(p) = patterns.get(idx) {
