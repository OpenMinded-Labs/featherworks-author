...
pub fn attempt_recovery(journal_path: &str) -> std::io::Result<Option<RecoveryReport>> {
    if !std::path::Path::new(journal_path).exists() { return Ok(None); }
    ...
    for line in rdr.lines() {
        ...
        let scene_id = v.get("scene_id").and_then(|s| s.as_str()).unwrap_or("").to_string();
        if scene_id.is_empty() { continue; }
        ...
        if snapshot {
            ...
            if let Some(cj) = v.get("content_json") {
                if let Ok(doc) = serde_json::from_value::<Node>(cj.clone()) {
                    scenes.insert(scene_id.clone(), RecoveredSceneState { scene_id, word_count, doc, from_snapshot: true });
                }
            }
        } else {
            inc_ct += 1;
            let entry = scenes.entry(scene_id.clone()).or_insert_with(|| RecoveredSceneState { scene_id: scene_id.clone(), word_count: 0, doc: Node::empty_doc(), from_snapshot: false});
            ...
            match pk {
                "insertText" => {
                    if let (Some(offset), Some(text)) = (v.get("offset").and_then(|x| x.as_u64()).map(|o| o as usize), v.get("text").and_then(|t| t.as_str())) {
                        if offset <= current_plain.len() { current_plain.insert_str(offset, text); }
                    }
                },
                "deleteRange" => {
                    if let (Some(start), Some(end)) = (v.get("start").and_then(|x| x.as_u64()).map(|o| o as usize), v.get("end").and_then(|x| x.as_u64()).map(|o| o as usize)) {
                        if start <= end && end <= current_plain.len() { current_plain.replace_range(start..end, ""); }
                    }
                },
                _ => { /* ignore unknown */ }
            }
    ...
    if scenes.is_empty() { return Ok(None); }
    Ok(Some(RecoveryReport { scenes: scenes.into_values().collect(), snapshot_count: snap_ct, incremental_count: inc_ct }))
}
...
pub fn recovered_to_patches(report: &RecoveryReport) -> Vec<(String, Patch)> {
    report.scenes.iter().map(|s| {
        (s.scene_id.clone(), Patch(PatchKind::FullReplace { new_doc: s.doc.clone() }))
    }).collect()
}
