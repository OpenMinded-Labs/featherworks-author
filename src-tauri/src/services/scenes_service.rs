...
pub enum SceneServiceError {
    #[error("DB error: {0}")]
    Db(#[from] database::DatabaseOperationError),
...
fn undo_map() -> &'static Mutex<HashMap<String, VecDeque<Patch>>> {
    UNDO_STACKS.get_or_init(|| Mutex::new(HashMap::new()))
}
fn redo_map() -> &'static Mutex<HashMap<String, VecDeque<Patch>>> {
    REDO_STACKS.get_or_init(|| Mutex::new(HashMap::new()))
}
fn counter_map() -> &'static Mutex<HashMap<String, usize>> {
    APPLY_COUNTERS.get_or_init(|| Mutex::new(HashMap::new()))
}
...
pub fn apply_patch(
    ...
    if let Some(journal_path) = current_journal_path() {
        let patch_kind_str = match &effective_patch.0 {
            PatchKind::FullReplace { .. } => "fullReplace",
            PatchKind::InsertText { .. } => "insertText",
            PatchKind::DeleteRange { .. } => "deleteRange",
        };
        ...
        if is_snapshot {
            if let serde_json::Value::Object(ref mut map) = obj {
                map.insert(
                    "content_json".into(),
                    serde_json::to_value(&new_doc).unwrap_or(serde_json::Value::Null),
                );
            }
        } else {
            if let serde_json::Value::Object(ref mut map) = obj {
