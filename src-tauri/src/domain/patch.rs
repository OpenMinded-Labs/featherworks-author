...
impl Patch {
    pub fn full(new_doc: Node) -> Self { Patch(PatchKind::FullReplace { new_doc }) }
    ...
    pub fn apply(&self, old: &Node) -> Node {
        match &self.0 {
            ...
            PatchKind::InsertText { offset, text } => {
                ...
                if *offset <= plain.len() { plain.insert_str(*offset, text); }
                Node::plain_text(&plain)
            }
            PatchKind::DeleteRange { start, end } => {
                let mut plain = crate::domain::doc::to_plain_text(old);
                if *start <= *end && *end <= plain.len() { plain.replace_range(*start..*end, ""); }
                Node::plain_text(&plain)
            }
        }
    }
...
pub fn diff_plain(old: &Node, new: &Node) -> Patch {
    ...
    if old_txt == new_txt { return Patch::full(new.clone()); }
    // Find common prefix
    let mut prefix = 0usize;
    let min_len = old_txt.len().min(new_txt.len());
    while prefix < min_len && old_txt.as_bytes()[prefix] == new_txt.as_bytes()[prefix] { prefix += 1; }
    // Find common suffix
    let mut suffix = 0usize;
    while suffix < (min_len - prefix)
        && old_txt.as_bytes()[old_txt.len() - 1 - suffix] == new_txt.as_bytes()[new_txt.len() - 1 - suffix]
    { suffix += 1; }
    ...
    match (old_mid.is_empty(), new_mid.is_empty()) {
        (true, false) => Patch(PatchKind::InsertText { offset: prefix, text: new_mid.to_string() }),
        (false, true) => Patch(PatchKind::DeleteRange { start: prefix, end: old_mid_end }),
        _ => {
...
mod tests {
    ...
    fn insert_text_patch() {
        ...
        let patch = Patch(PatchKind::InsertText { offset: 6, text: "Brave ".into() });
        let applied = patch.apply(&old);
        assert_eq!(crate::domain::doc::to_plain_text(&applied), "Hello Brave World");
    }
